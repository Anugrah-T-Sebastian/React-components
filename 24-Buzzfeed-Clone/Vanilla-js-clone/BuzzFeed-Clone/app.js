...
const handleClick = (questionId, chosenAnswer) => {
  ...
  if (itemToRemove > -1) unansweredQuestions.splice(itemToRemove, 1);
  disableQuestionBlock(questionId, chosenAnswer);
  const lowestQuestionedId = Math.min(...unansweredQuestions);
  location.href = "#" + lowestQuestionedId;
...
const showAnswer = () => {
  ...
  if (!result) result = answers[0]; //use 1st combination as default
  console.log(result);
  const answerBlock = document.createElement("div");
